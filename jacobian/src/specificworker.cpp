...
bool SpecificWorker::setParams(RoboCompCommonBehavior::ParameterList params)
{
	...
	innerModel = std::make_shared<InnerModel>("/home/robocomp/robocomp/files/innermodel/betaWorldArm4DOF.xml");
	...
	joints << "shoulder_right_1"<<"shoulder_right_2"<<"elbow_right" << "wrist_right_1" << "wrist_right_2";
	...
	connect(pushButton_front, SIGNAL(pressed()), this, SLOT(fronSlot()));
...
void SpecificWorker::moveArm( float dx, float dy, float dz)
{
	...
	jacobian = jacobian * (T)0.001;
	//jacobian.print("jac");
	RoboCompJointMotor::MotorGoalVelocityList vl;
	RoboCompJointMotor::MotorGoalPositionList pl;
	...
	try
	{
		...
		QVec incs = jacobian.transpose() * error;	
		incs.print("incs");
	...
	{ 
		...
		jointmotor_proxy->setSyncPosition(pl);
	}
	catch(const Ice::Exception &e)
	{	std::cout << e.what() << std::endl;}
}
...
void SpecificWorker::SimpleArm_home()
{
	goHome();
